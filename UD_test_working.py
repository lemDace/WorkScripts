...
def count_matching_files(input_folder: Path) -> int:
    """
    Count total files that will actually be processed according to current logic:
    - One file per UDxx per subfolder
    - Prefer .csv over .xlsx if both exist
    """
    total = 0
    for sub in input_folder.iterdir():
        if not sub.is_dir():
            continue
        for ud in UD_TABLES:
            # find matching files in this subfolder for this UD
            matching_files = [
            ...
            if matching_files:
                total += 1  # only count 1 per UD per subfolder
    return total
...
def worker_process(input_folder: Path, output_folder: Path, prefix: str,
    ...
    files_log_path = output_folder / "MasterUD_FilesUsed_Log.txt"
    ...
    for ud in UD_TABLES:
        ...
        for sub in sorted(input_folder.iterdir()):
            ...
            if sub.resolve() == output_folder.resolve():
                continue
            ...
            # Log what was actually processed
            log_write(files_log_path, f"{ud} - {sub.name} -> {preferred_file.name}")
